 ...
 *
 * The untextured sculpt (no UVs, two meshes) gets generated box-projected
 * UVs and a procedural emissive texture — dark metal with glowing blue
 * data veins (the backend-baas key-art look). The outer ring rotates
 * uniformly by default; pointer devices can stop it (hover) and fling it
 * — opposite direction or faster — with inertia easing home. Touch: drag
 * to spin. Chrome cubes orbit the core.
 *
 * Everything is defensive: traverse-based mesh collection, guarded UV
 * generation (falls back to chrome-only if it fails), a size guard so the
 * canvas never mounts at 0×0 (NaN matrices), and onFail so the wrapper
 * can show the key-art image if 3D truly can't run.
 */

/** per-triangle planar (box) UVs — no DCC unwrapping needed */
function boxUnwrap(geo: THREE.BufferGeometry): boolean {
  try {
    ...
    for (let t = 0; t < triCount; t++) {
      ...
      const project = (v: THREE.Vector3): [number, number] =>
        ax > ay && ax > az ? [v.z, v.y] : ay > az ? [v.x, v.z] : [v.x, v.y];
...
export function ServerScene({ onFail }: { onFail?: () => void }) {
  ...
  useEffect(() => {
    ...
    const initScene = (): (() => void) | undefined => {
      ...
      scene.add(coreGroup, ringGroup, orbiters);
      ...
      new GLTFLoader().load(
        ...
        (gltf) => {
          if (disposed) return;
          try {
            // traverse-collect (robust against nesting), widest = ring
            ...
            const spanX = (m: THREE.Mesh) =>
              new THREE.Box3().setFromObject(m).getSize(new THREE.Vector3()).x;
            const sorted = [...meshes].sort((a, b) => spanX(b) - spanX(a));
            const ringMesh = sorted[0];
            const coreMesh = sorted[1] ?? null;

            const fit = (m: THREE.Mesh, target: number) => {
              const hasUv = boxUnwrap(m.geometry);
              const diag = new THREE.Box3().setFromObject(m).getSize(new THREE.Vector3()).length();
              const k = target / Math.max(diag, 0.001);
              m.scale.setScalar(k);
              ...
              if (hasUv) mat.emissiveMap = emis; // chrome-only fallback if UVs failed
            ...
            if (coreMesh) coreGroup.add(fit(coreMesh, 1.55));

            // orbiting chrome cubes
            ...
            if (reduced) renderer.render(scene, camera); // static frame with models in
          } catch (err) {
            console.error("[server-scene] model setup failed", err);
            onFail?.();
          }
        },
        undefined,
        (err) => {
          console.error("[server-scene] load failed", err);
          onFail?.();
        },
      );
      ...
      const tick = () => {
        ...
        // core + ring glow breathe
        [coreGroup, ringGroup].forEach((g, i) => {
